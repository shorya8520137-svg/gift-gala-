...
```json
{
  "product_id": 1,
  "quantity": 2,
  "variant_details": {"color": "black", "size": "large"}
}
```
...
### 1. Create Order
Create a new order from cart items or direct purchase.
...
|-----------|------|---------|-------------|
| `page` | integer | 1 | Page number for pagination |
| `limit` | integer | 10 | Number of orders per page |
  ...
  "data": [
    {
      ...
      "delivered_at": "2025-01-05T15:30:00.000Z",
...
### 5. Track Order
Track an order using order number (public endpoint).
...
When order status changes, webhooks can be sent to notify external systems.
...
- `id` - Primary key
- `cart_id` - Foreign key to user_carts
- `product_id` - Foreign key to website_products
- `quantity` - Item quantity
- `variant_details` - JSON object for product variants
- `created_at` - Creation timestamp
- `updated_at` - Last update timestamp
...
- `total_amount` - Subtotal before tax/shipping
- `tax_amount` - Tax amount
- `shipping_amount` - Shipping cost
- `discount_amount` - Discount applied
- `final_amount` - Total amount to pay
- `status` - Order status
...
- `product_image` - Product image URL at time of order
...
*API Documentation updated: February 2, 2026*